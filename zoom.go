...
func MSave(models []Model) error {
	t := newTransaction()
	p, err := t.addPhase("MSave", nil, nil)
	if err != nil {
		return err
	}
...
func MFindById(modelNames, ids []string) ([]Model, error) {
	...
	t := newTransaction()
	p, err := t.addPhase("MFindById", nil, nil)
	if err != nil {
		return nil, err
	}
...
func ScanById(id string, model Model) error {
	...
	// start a transaction
	t := newTransaction()
	p, err := t.addPhase("ScanById", nil, nil)
	if err != nil {
		return err
	}
	p.scanModel(mr, nil)
...
func MScanById(ids []string, models interface{}) error {
	...
	t := newTransaction()
	p, err := t.addPhase("MScanById", nil, nil)
	if err != nil {
		return err
	}
	for i := 0; i < len(ids); i++ {
...
func Delete(model Model) error {
	t := newTransaction()
	p, err := t.addPhase("Delete", nil, nil)
	if err != nil {
		return err
	}
...
func MDelete(models []Model) error {
	t := newTransaction()
	p, err := t.addPhase("MDelete", nil, nil)
	if err != nil {
		return err
	}
...
func DeleteById(modelName string, id string) error {
	t := newTransaction()
	p, err := t.addPhase("DeleteById", nil, nil)
	if err != nil {
		return err
	}
...
func MDeleteById(modelNames []string, ids []string) error {
	...
	t := newTransaction()
	p, err := t.addPhase("MDeleteById", nil, nil)
	if err != nil {
		return err
	}
