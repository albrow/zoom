...
-- find_models_by_reverse_sorted_set_ids is a lua script that takes the
-- following arguments:
...
-- *reverse* order of the sorted set. It returns an array of arrays where each
-- array contains the fields for a particular model.
...
local ids = redis.call('ZREVRANGE', setKey, 0, -1)
local models = {}
if #ids > 0 then
